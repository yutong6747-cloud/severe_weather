...
from ultralytics.nn.modules.custom import ChannelAttention, SobelConv, FGM, OmniKernel, EIEStem, SPDConv, ECABlock, CSPOmniKernel, Scale, Conv_GN, DEConv_GN, Detect_LSDECD
...
# Optional module symbols that are not shipped in this trimmed project.
BRA = CSPAMSKF = GEIP = MSEE = Index = MSEAF = ScalCat = Scal3DC = DEConv = SRepDDetect = None
...
C3K2_CLASS = (C3k2_Faster, C3k2_ODConv, C3k2_Faster_EMA, C3k2_DBB, C3k2_DeepDBB, C3k2_WDBB, C3k2_CloAtt, C3k2_SCConv, C3k2_ScConv, C3k2_EMSC, C3k2_EMSCP, C3k2_KW, C3k2_DySnakeConv, C3k2_DCNv2, C3k2_DCNv3,
              ...
              C3k2_GroupMambaBlock, C3k2_MambaVision, DSC3k2, C3k2_wConv, C3k2_FourierConv, C3k2_GLVSS
              )
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # Special handling for GEIP and Index layers
        if m is GEIP:
            # GEIP args are channel sizes [c2, c3, c4, c5] before width scaling.
            args = [make_divisible(min(arg, max_channels) * width, 8) for arg in args]
            m_ = m(*args)
            ...
            layers.append(m_)
            ch.append(args[1:])  # GEIP outputs [P3e, P4e, P5e] as a list
            continue
        elif m is Index:
            # Index: extracts a specific index from a list output
            index = args[0] if args else 0
            if isinstance(f, int) and isinstance(ch[f], list):
                c2 = ch[f][index]
            elif isinstance(f, list) and isinstance(ch[f[0]], list):
                c2 = ch[f[0]][index]
            ...
            m_ = m(*args)
            t = str(m)[8:-2].replace('__main__.', '')
            m_.np = sum(x.numel() for x in m_.parameters()) if hasattr(m_, 'parameters') else 0
            m_.i, m_.f, m_.type = i, f, t
            if verbose:
                LOGGER.info(f"{i:>3}{str(f):>20}{n_:>3}{m_.np:10.0f}  {t:<45}{str(args):<30}")
            save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)
        ...
        if m in ((
            ...
            ) + C3K2_CLASS + A2C2F_CLASS + C2PSA_CLASS + (C2fDCB,)):
            ...
            # GEIP takes [p2, p3, p4, p5] as input and outputs [p3_e, p4_e, p5_e]
            c2 = args[1]  # c3 (P3 output channel)
...
def yaml_model_load(path):
    ...
    if not d.get("scale"):  # 保留 yaml 内显式 scale（如 scale: s），勿被路径猜测覆盖
