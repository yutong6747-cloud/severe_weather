...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'C3k2_MambaOut_SFSC', 'C3k2_PSFSConv', 'C3k2_FasterSFSC', 'FCM', 'FCM_1', 'FCM_2', 'FCM_3', 'Pzconv', 'C3k2_GroupMamba', 'C3k2_GroupMambaBlock', 'C3k2_MambaVision', 'C3k2_wConv', 'wConv2d', 'PST', 'C3k2_FourierConv', 'FourierConv', 'C3k2_GLVSS','ECABlock',
...
# -------------------------------------恶劣天气第二个改进点中的SPDConv-HF------------------------------------------------------

class SPDConv(nn.Module):
    """
    SPDConv + 高频边缘增强，用于恶劣天气下的小目标检测.
    保持 (inc, ouc, dimension=1) 接口不变.
    """
    def __init__(self, inc, ouc, dimension=1, use_highfreq=True):
        ...
        if self.use_highfreq:
            # depthwise 拉普拉斯卷积：突出边缘和细节
            self.lap = nn.Conv2d(
                inc, inc, kernel_size=3, padding=1,
                groups=inc, bias=False
            )
            ...
            with torch.no_grad():
                w = lap_kernel.view(1, 1, 3, 3).repeat(inc, 1, 1, 1)
                self.lap.weight.copy_(w)

            # 用 2x2 平均池化把 hf 下采样一半，保证和 SPD 分支同尺寸
            self.hf_pool = nn.AvgPool2d(kernel_size=2, stride=2)

            in_channels_conv = inc * 4 + inc  # SPD 4 倍 + 高频分支
        else:
            self.lap = None
            self.hf_pool = None
    ...
    def forward(self, x):
        ...
        if self.use_highfreq:
            # 高频边缘特征 + 2x 下采样，保证尺寸匹配
            hf = self.lap(x)           # [B, C, H, W]
            hf = self.hf_pool(hf)      # [B, C, H/2, W/2]
...
class ChannelAttention(nn.Module):
    """
    通道注意力模块（类似 SE），保留第 4 点改进
    """
    def __init__(self, channels: int, reduction: int = 16):
        super().__init__()
        mid_channels = max(channels // reduction, 4)  # 防止过小
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
...
class EIEStem(nn.Module):
    """
    轻量版 EIEStem（保留注意力）：
    - 整体保持 4x 下采样：conv1(stride=2) + conv2(stride=2)
    - Sobel 分支保留
    - pool 分支恢复为最简单的 pad + maxpool
    - concat 后保留 ChannelAttention（第 4 点）
    - 保留一个轻量残差 shortcut（第 5 点）
    """
    def __init__(self, inc: int, hidc: int, ouc: int) -> None:
        ...
        # 精简版 pool 分支：pad + maxpool，不再加 1x1 Conv 等
        self.pool_branch = nn.Sequential(
            nn.ZeroPad2d((0, 1, 0, 1)),
            nn.MaxPool2d(kernel_size=2, stride=1, padding=0, ceil_mode=True)
        )
        ...
        # 残差：从 conv1 输出 (hidc, H/2, W/2) 到 (ouc, H/4, W/4)
...
#-----------------------------------恶劣天气第二个改进点的   ECA+CSPOmniKernel   -----------------------------------------------------------------
class ECABlock(nn.Module):
    """
    极轻量通道注意力（ECA），几乎不增加计算量.
    """
...
class CSPOmniKernel(nn.Module):
    """
    原 CSPOmniKernel + 轻量 ECA 注意力.
    保持 __init__(dim, e=0.25) 接口不变，兼容 yaml 写法：[]
    """
    def __init__(self, dim, e=0.25, use_eca=True):
        ...
        if self.use_eca:
            self.attn = ECABlock(dim)
        else:
            self.attn = None

    def forward(self, x):
        feat = self.cv1(x)
        ok_channels = int(self.cv1.conv.out_channels * self.e)
        id_channels = feat.shape[1] - ok_channels  # 防止浮点误差

        ok_branch, identity = torch.split(feat, [ok_channels, id_channels], dim=1)
...
#---------------------------------------------------R_Attention（backbone部分，提高树冠在复杂背景下的区分度）--------------------------------------------------------
# class R_Attention(nn.Module):
#    """
#    Region-level Attention: 针对树冠圆形特征设计的区域注意力
#    """
#    def __init__(self, channels):
#        super().__init__()
#        # 使用 7x7 大核卷积提取局部区域特征，模拟树冠的整体轮廓
#        self.region_conv = nn.Conv2d(channels, channels, kernel_size=7, padding=3, groups=channels)
...
#        # 生成通道与空间融合的注意力权重
#        att_weight = self.gate(self.fc(region_feat))
#        return x * att_weight

#---------------------------------------------------ASFF（neck部分，不同生长阶段树木的尺度波动）--------------------------------------------------------
#class ASFF_Layer(nn.Module):
#    """
#    Adaptive Spatial Feature Fusion: 自适应空间特征融合
#    用于在检测头之前平衡大树和小树的特征权重
#    """
#    def __init__(self, c1, c2): # c1 是输入通道列表, c2 是输出通道
#        super().__init__()
#        # 假设从不同层级传来的通道需要缩放到一致
#        self.conv1 = Conv(c1[0], c2, 1)
#        self.conv2 = Conv(c1[1], c2, 1)
#        self.conv3 = Conv(c1[2], c2, 1)
#    def __init__(self, level, multiplier=1.0):
#        super().__init__()
#        self.level = level
#        self.dim = [int(256*multiplier), int(512*multiplier), int(1024*multiplier)]
#        curr_dim = self.dim[self.level]

        # 学习三个层级的权重
#        self.weight_level_0 = nn.Conv2d(curr_dim, 1, kernel_size=1)
#        self.weight_level_1 = nn.Conv2d(curr_dim, 1, kernel_size=1)
#        self.weight_level_2 = nn.Conv2d(curr_dim, 1, kernel_size=1)

#    def forward(self, x0, x1, x2):
        # x0, x1, x2 分别代表 P3, P4, P5 融合后的特征图
#        w0 = self.weight_level_0(x0)
#        w1 = self.weight_level_1(x1)
#        w2 = self.weight_level_2(x2)

        # 归一化权重
#        combined_weights = torch.cat([w0, w1, w2], dim=1)
#        combined_weights = F.softmax(combined_weights, dim=1)

        # 加权融合
#       out = x0 * combined_weights[:, 0:1, :, :] + \
#              x1 * combined_weights[:, 1:2, :, :] + \
#              x2 * combined_weights[:, 2:3, :, :]
#        return out
