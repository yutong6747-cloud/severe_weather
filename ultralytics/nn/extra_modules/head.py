...
#             x[i] = y
#         # print(f"[LOG] ✅ Using modified Detect_LSCD with SEBlock | nc={self.nc}, hidc={self.share_conv[0].conv.in_channels}")  可视化修改
#         if self.training:
#             return x
#
#         # Inference path
...
class Detect_LSDECD(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        # 回归/分类头：保持不变
        self.cv2 = nn.Conv2d(hidc // 2, 4 * self.reg_max, 1)  # 输入改半通道（不增算力，反降）
        self.cv3 = nn.Conv2d(hidc // 2, self.nc, 1)
...
class Conv_GN(nn.Module):
    """
    Conv + GroupNorm + SiLU
    如果你的工程里已经定义过 Conv_GN，可以删除这个类，直接用原来的。
    """
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, num_groups=16):
        super().__init__()
        if p is None:
            p = k // 2
        ...
        # 保证 num_groups 能整除通道数，否则退化为 1 组
        gn_groups = num_groups if c2 % num_groups == 0 else 1
...
class DEConv_GN(nn.Module):
    ...
    def __init__(self, channels, kernel_size=3, stride=1, num_groups=16):
        ...
        # GroupNorm
        gn_groups = num_groups if channels % num_groups == 0 else 1
        self.gn = nn.GroupNorm(gn_groups, channels)
...
#         # 每层先做 1x1 统一通道
#         self.conv = nn.ModuleList(Conv_GN(x, hidc, 1) for x in ch)

#         # 共享细节增强模块：两层 DEConv_GN 串联
#         self.share_conv = nn.Sequential(
#             DEConv_GN(hidc),
#             DEConv_GN(hidc)
#         )
...
#         # 注意：这里仍然是对同一个 cv3.bias 重复赋值，最终只保留最后一次
#         # 如果你想真正按不同检测层分别初始化，需要把 cv3 改成 ModuleList
#         for s in self.stride:
#             prior = 5 / m.nc / (640 / s) ** 2
#             m.cv3.bias.data[: m.nc] = math.log(prior)
...
#---------------------------------------------------TreeDecoupledHead--------------------------------------------------------
# class TreeDecoupledHead(nn.Module):
#     """
#     借鉴自 YOLOX 的解耦头结构，专门为树种识别定制。
#     """
...
#             Conv(ch, ch, 3)
#         )
#         self.reg_pred = nn.Conv2d(ch, 4, 1) # x, y, w, h
#         self.obj_pred = nn.Conv2d(ch, 1, 1) # 置信度
