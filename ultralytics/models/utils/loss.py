...
class HierarchicalLossWrapper(nn.Module):
    def __init__(self, base_loss, iou_threshold=0.7, penalty_factor=2.0):
        super().__init__()
    ...
    def forward(self, preds, targets):
        base_loss_val = self.base_loss(preds, targets)

        # 取出预测框和标签
        pred_boxes = preds[:, :, :4]   # xywh
        pred_cls = preds[:, :, 5]      # 类别索引
        gt_boxes = targets[:, 2:6]     # xywh
        gt_cls = targets[:, 1]

        penalty_loss = 0.0
        for i, (p_box, p_cls) in enumerate(zip(pred_boxes, pred_cls)):
            if int(p_cls) in [1, 2]:  # broken / flashover
                inside = False
                for g_box, g_cls in zip(gt_boxes, gt_cls):
                    if g_cls == 0:  # insulator
                        if self.iou_xywh(p_box, g_box) > self.iou_threshold:
                            inside = True
                            break
                if not inside:
                    penalty_loss += 1.0

        total_loss = base_loss_val + self.penalty_factor * penalty_loss
        return total_loss
    ...
    def iou_xywh(box1, box2):
        x1, y1, w1, h1 = box1
        x2, y2, w2, h2 = box2
        b1_xyxy = [x1-w1/2, y1-h1/2, x1+w1/2, y1+h1/2]
        b2_xyxy = [x2-w2/2, y2-h2/2, x2+w2/2, y2+h2/2]
        xi1 = torch.max(b1_xyxy[0], b2_xyxy[0])
        yi1 = torch.max(b1_xyxy[1], b2_xyxy[1])
        xi2 = torch.min(b1_xyxy[2], b2_xyxy[2])
        yi2 = torch.min(b1_xyxy[3], b2_xyxy[3])
